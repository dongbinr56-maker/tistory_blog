...
def _copy_page(drafts: list[dict[str, Any]]) -> str:
...
if(!drafts.length)detail.innerHTML='<p>아직 생성된 초안이 없습니다.</p>';
    ...
    return template.replace("__DRAFT_PAYLOAD__", payload)
...
def build_site(root: Path, site: dict[str, Any]) -> None:
    ...
    (docs / "index.html").write_text(_copy_page(drafts), encoding="utf-8")
