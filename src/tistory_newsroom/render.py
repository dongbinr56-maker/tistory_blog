...
function heroDownloadFilename(url,date){try{const match=new URL(url,window.location.href).pathname.match(/\\.([a-z0-9]+)$/i);return `tistory-대표이미지-${date}.${match?match[1]:'svg'}`;}catch(_error){return `tistory-대표이미지-${date}.svg`;}}
...
function renderDetail(raw){
  ...
  const heroDownload=raw.images?.hero?.url?`<div class="field"><b>대표 이미지</b><p>대표 이미지의 제목 텍스트까지 포함한 원본 파일입니다.</p><button class="copy" id="download-hero" type="button">대표 이미지 다운로드</button></div>`:'';
