...
function thumbnailDownloadFilename(date){return `tistory-대표-썸네일-${date}.png`;}
...
async function downloadThumbnail(){
  const thumbnail=current?.images?.thumbnail;if(!thumbnail?.url){status('다운로드할 대표 썸네일이 없습니다.');return;}
  const button=detail.querySelector('#download-thumbnail');if(button)button.disabled=true;
  try{const response=await fetch(thumbnail.url,{cache:'no-store'});if(!response.ok)throw new Error(`HTTP ${response.status}`);downloadBlob(await response.blob(),thumbnailDownloadFilename(current.date));status('별도로 생성한 대표 썸네일 PNG를 다운로드했습니다.');}
  catch(_error){const link=document.createElement('a');link.href=thumbnail.url;link.download=thumbnailDownloadFilename(current.date);link.target='_blank';link.rel='noopener';document.body.append(link);link.click();link.remove();status('직접 다운로드가 막혀 대표 썸네일을 새 창에서 열었습니다.');}
...
function renderDetail(raw){
  ...
  const thumbnailDownload=raw.images?.thumbnail?.url?`<div class="field"><b>블로그 대표 썸네일</b><p>본문의 대표 이미지·이슈 1·2·3 이미지와 별도로 생성한 1200×630 PNG입니다. 티스토리 대표 이미지를 등록할 때 사용하세요.</p><button class="copy" id="download-thumbnail" type="button">대표 썸네일 PNG 다운로드</button></div>`:'';
  ...
  detail.querySelector('#download-thumbnail')?.addEventListener('click',downloadThumbnail);
