...
async function downloadHero(){
  const hero=current?.images?.hero;if(!hero?.url){status('다운로드할 대표 이미지가 없습니다.');return;}
  const button=detail.querySelector('#download-hero');if(button)button.disabled=true;
  try{const response=await fetch(hero.url,{cache:'no-store'});if(!response.ok)throw new Error(`HTTP ${response.status}`);downloadBlob(await response.blob(),heroDownloadFilename(hero.url,current.date));status('텍스트가 포함된 대표 이미지를 다운로드했습니다.');}
  catch(_error){const link=document.createElement('a');link.href=hero.url;link.download=heroDownloadFilename(hero.url,current.date);link.target='_blank';link.rel='noopener';document.body.append(link);link.click();link.remove();status('직접 다운로드가 막혀 원본 이미지를 새 창에서 열었습니다.');}
...
function renderDetail(raw){
  ...
  const heroDownload=raw.images?.hero?.url?`<div class="field"><b>대표 이미지</b><p>제목 텍스트까지 포함한 1200×630 PNG 원본입니다.</p><button class="copy" id="download-hero" type="button">대표 이미지 PNG 다운로드</button></div>`:'';
