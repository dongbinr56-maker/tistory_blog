...
가장 중요한 규칙:
- 독자는 AI를 처음 접하는 일반인부터 실무 AI 엔지니어까지다. 각 이슈를 plain_explanation에서 먼저 쉬운 비유와 일상어로 2~3문장 설명한 뒤, technical_details에서 엔지니어가 궁금해할 모델·데이터·학습·추론·배포·하드웨어·오픈소스 관점을 구체적으로 설명합니다.
- 원문을 복제·번역·문장 치환하지 말고, 사실을 짧게 요약한 뒤 독자에게 새로운 가치를 주는 분석을 작성합니다.
- 각 이슈에서 what_happened(확인된 사실), plain_explanation(일반인 설명), why_it_matters(영향), technical_details(확인된 기술 정보), editorial_take(엔지니어 관점), reader_action(직접 해볼 점검)을 명확히 분리합니다.
- technical_details에는 입력의 verification 정보로 확인된 값만 씁니다. 모델 구조·학습 데이터·라이선스·성능 수치·GitHub 스타 수·Hugging Face 다운로드 수는 입력에 없으면 "공개 자료에서 확인하지 못함"이라고 쓰고 절대 추정하지 않습니다.
- verification_notes에는 무엇을 원문, GeekNews 소개 페이지, GitHub/Hugging Face 공식 페이지에서 확인했는지 짧게 적습니다.
...
- article_count_note에는 "오늘은 기준을 충족한 기사가 {article_count}건이었다"는 문장을 포함합니다. 3개 미만인 경우 부족한 항목을 만들지 않았다는 이유를 자연스럽게 덧붙입니다.
...
{{
  ...
  "article_count_note": "오늘의 기사 개수 고지",
  "intro": "2~3문단 도입",
  "sections": [{{
    "headline": "소제목",
    "source_ids": ["입력 id"],
    ...
    "technical_details": "확인된 기술적 사실과 미확인 범위",
    "editorial_take": "작성자 분석",
    "reader_action": "독자 행동 제안",
    "verification_notes": "확인한 원문·공식 페이지 범위"
  }}],
  "closing": "마무리",
...
def generate_demo(date: str, sources: list[SourceItem], site: dict[str, Any]) -> Draft:
    ...
    for source in sources:
        ...
        official = source.official_url or "공식 프로젝트 페이지는 원문에서 별도로 확인되지 않았습니다"
        sections.append(
            {
                ...
                "technical_details": (
                    f"확인된 프로젝트 또는 원문 이름은 {project_name}입니다. 공식 확인 경로는 {official}입니다. "
                    f"라이선스는 {source.verification.get('license') or '공개 자료에서 확인하지 못함'}입니다. "
                    "모델 구조, 학습 데이터, 벤치마크 수치처럼 입력에 없는 값은 이 초안에서 단정하지 않습니다."
                ),
                "editorial_take": (
                ...
                "verification_notes": "원문 제목·출처·URL을 확인했고, 공식 프로젝트 URL과 라이선스는 원문에 연결된 경우에만 기록했습니다.",
            }
        )
    titles = [
    ...
    raw = {
        ...
        "article_count_note": f"오늘은 기준을 충족한 기사가 {len(sources)}건이었다. 부족한 항목을 억지로 채우지 않고 확인된 기사만 다룬다.",
        "intro": (
            "오늘의 초점은 AI 모델을 직접 만들거나, 팀의 제품에 붙이거나, 오픈소스 모델을 평가하는 사람에게 실제로 중요한 변화입니다. "
            "뉴스를 넓게 훑는 대신 모델·에이전트·컴퓨팅·오픈소스의 연결점만 골랐습니다.\n\n"
            "각 이슈는 먼저 쉬운 말로 설명하고, 이어서 엔지니어가 확인해야 할 기술적 사실과 아직 확인되지 않은 범위를 분리합니다. "
