...
1. 복사 페이지에서 날짜를 고릅니다. 기본 `HTML` 탭에는 티스토리에 바로 붙여넣을 수 있는 실제 본문 소스가 표시되고, `View` 탭에는 같은 소스의 렌더링 결과가 표시됩니다. `대표 썸네일 PNG 다운로드`를 누르면 본문 대표 이미지·이슈 이미지와 별도로 생성된 1200×630 PNG 파일을 받을 수 있습니다.
