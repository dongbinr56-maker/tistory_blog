...
매일 오전 07시(KST)에 **요즘IT·GeekNews·Hacker News의 최근 24시간 AI 모델·에이전트·오픈소스 기사**를 검증하고, **검토 가능한 티스토리 HTML 초안**으로 재구성하는 GitHub Actions 프로젝트입니다. 자동 발행은 하지 않습니다. 티스토리의 공식 글 작성 Open API가 종료되어 있고, 뉴스의 무단 재게시·무검토 자동 발행은 품질과 정책 측면에서 적절하지 않기 때문입니다.
...
```mermaid
flowchart LR
  A[07:00 KST GitHub Actions] --> B[요즘IT·GeekNews·Hacker News 최근 24시간 수집]
  B --> C[원문·게시시각·GitHub/HF 공식 페이지 검증]
  C --> D[과거 선택 URL 제외 후 3~4건 선별: GitHub/HF 프로젝트 1건 이상]
  D --> E[Gemini 초안 생성 + 상투 표현·중복 재작성 루프]
  E --> F[애드센스 품질 게이트 + 소스 장애 경고]
...
docs/tistory/YYYY-MM-DD.html               # 검토용 본문 문서(noindex 래퍼 포함)
docs/tistory/YYYY-MM-DD.json               # 제목 후보·태그·검토 정보
docs/tistory/assets/YYYY-MM-DD/             # 대표(썸네일) 이미지 1장과 이슈별 이미지
...
1. 복사 페이지에서 날짜를 고릅니다. 기본 `HTML` 탭에는 티스토리에 바로 붙여넣을 수 있는 실제 본문 소스가 표시되고, `View` 탭에는 같은 소스의 렌더링 결과가 표시됩니다. `대표 이미지 다운로드`를 누르면 제목 텍스트가 들어간 PNG를 받아 티스토리 글쓰기의 **대표 이미지(썸네일)**로 등록합니다. 대표 이미지는 본문에는 들어가지 않습니다.
2. `View`에서 이미지·본문·원문 링크를 확인합니다. 도입부 아래의 **작성자 코멘트는 AI가 기본으로 작성**해 본문에 포함돼 있고(경험 날조 금지, 제안·견해만), 직접 한마디 하고 싶은 날엔 검토 페이지의 코멘트 입력란에 쓰면 그 내용으로 교체됩니다.
3. `HTML` 탭의 **본문 HTML 복사**를 누릅니다. noindex 래퍼를 제외한 `<article>` 본문만 복사되므로 티스토리 글쓰기 **HTML 모드**에 그대로 붙여넣으면 됩니다. 제목은 AI가 그날 내용으로 지은 후보 중에서 고르고(고정 `[AI 뉴스룸]` 포맷은 예비 후보 1개로만 제공), 모든 원문 링크·사실·분석·문체·내부 링크를 검토합니다. 이미지는 GitHub Pages 주소를 참조하므로, 장기 운영 시 티스토리에 직접 업로드해 교체하는 편이 안전합니다.
...
- 요즘IT·GeekNews·Hacker News(100점 이상, 공식 Algolia API)에서 최근 24시간 이내에 실제 페이지로 확인한 기사만 수집
- 기사만으로 3건이 안 되면 관련성이 있는 공식 1차 자료로 보완하되, 글마다 GitHub/Hugging Face 프로젝트를 기계적으로 끼워 넣지 않음
- 모델·에이전트·컴퓨터 비전·오픈소스·개발 도구·온디바이스/가속기 중심의 관련성 필터
- 각 이슈의 쉬운 설명, 확인된 기술 정보, 엔지니어 관점, 검증 메모, 원문 링크
- 모든 초안에 `evidence_mode`(`hands_on`/`source_analysis`), `execution_status`(`performed`/`partial`/`not_performed`), `evidence_artifacts`를 기록하고 본문 상단에 검증 방식·실행 여부·확인일·사용 자료를 표시
- 만들기·실습·테스트를 약속한 글은 실제 실행 없이 발행하지 않으며, 문서 기반 글은 확인 범위와 직접 검증하지 못한 한계를 출처별로 기록
- 여러 글에서 동일한 비기능성 소제목 묶음을 반복하지 않음
- 선정 이슈마다 이미지 1장과 대표 이미지 1장을 만듦
- 원문 복사·단순 치환을 막는 재구성 프롬프트와 출처 보관
- 상투 표현·반복 비유·중복 문단을 패턴 검사와 Gemini 감수 패스로 재작성하고, 그래도 남으면 차단 대신 검토 화면에 문체 경고로 표시(발행 전 사람이 해당 문장만 다듬으면 됨)
- 수집 실패나 기사형 소스 0건은 품질 보고서 경고·검토 화면·Actions 요약에 표시(기사 없는 날은 발행 보류 권장)
- 자동 생성 글의 무검토 게시 금지(`manual_review_required: true`). 작성자 코멘트는 AI가 기본 작성(경험 날조 금지)하되 검토 페이지에서 직접 쓴 코멘트로 교체 가능
- GitHub Pages의 초안·검토 화면 전체를 noindex로 색인 차단해 티스토리 글이 원본이 되게 유지
- 제목·태그 중복, 출처 추적, 금지 주제·광고 클릭 유도 문구 검사. 글자 수와 출처 개수는 승인 기준이 아니라 보조 경고로만 사용
...
실사이트의 공개 글·공지 수, 내부 404, 비공개 대상 노출, 검증 표시와 대표 글의 증거 자료는 다음 명령으로 확인합니다. 모든 조건이 통과하기 전에는 AdSense 재검토를 요청하지 않습니다.

```powershell
tistory-newsroom audit-live `
  --base-url https://jammini-info.tistory.com/ `
  --expected-posts 20 `
  --expected-notices 4
```
...
다운로드 폴더의 `./이미지.png` 경로는 로컬 미리보기 전용이며 티스토리 에디터가 파일을 업로드해 주지 않습니다. 이미지 3장이 들어간 원고는 공개 자산 경로로 복사하고 HTTPS 주소를 넣은 별도 발행본을 만드세요.

```powershell
tistory-newsroom prepare-publish `
  --html "deliverables/2026-07-21/ai-work-automation/article-local-preview.html" `
  --asset-dir "docs/tistory/assets/deliveries/2026-07-21/ai-work-automation" `
  --asset-base-url "https://dongbinr56-maker.github.io/tistory_blog/tistory/assets/deliveries/2026-07-21/ai-work-automation" `
  --output "deliverables/2026-07-21/ai-work-automation/article-tistory-paste.html"
```

공개 자산을 배포한 뒤 세 URL이 HTTP 200과 `image/*`로 응답하는지 확인합니다. 티스토리 자체 대표이미지 지정이 필요하면 `cover.png`는 에디터에서 별도로 업로드합니다.
